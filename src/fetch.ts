...
function ghApiArray(endpoint: string): unknown[] {
  ...
  while (true) {
    const items = ghApi(`${endpoint}${sep}per_page=100&page=${page}`) as unknown[];
    results.push(...items);
...
export function fetchPRData(repo: string, prNumber: number): PRData {
  ...
  return {
    ...
    topComments: ghApiArray(`${base}/issues/${prNumber}/comments`) as IssueComment[],
    reviews: ghApiArray(`${base}/pulls/${prNumber}/reviews`) as Review[],
    diffComments: ghApiArray(`${base}/pulls/${prNumber}/comments`) as DiffComment[],
  };
}
